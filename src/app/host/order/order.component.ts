      ...
      confirmButtonText: 'Nhận đơn!'
        ...
        if (result.isConfirmed) {
          this.orderService.changeStatusOrderDone(id).subscribe(() => {
              this.notifi.showMessage('success', 'Thành công!', 'Đồng ý đơn hàng');
              this.getAllOrderProcessingByUserId();
            }, error => this.notifi.showMessage('error', 'Done!', 'Xảy ra lỗi')
          );
        }
      }
    );
    ...
    Swal.fire({
      ...
      confirmButtonText: 'Hủy đơn!'
        ...
        this.orderService.changeStatusOrderCanceled(id).subscribe(() => {
            this.notifi.showMessage('success', 'Thành công!', 'Đã từ chối đơn hàng');
            this.getAllOrderProcessingByUserId();

          }, error => this.notifi.showMessage('error', 'Canceled!', 'Xảy ra lỗi')
        );
      }
    );
  }
