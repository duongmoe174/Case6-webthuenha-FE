      ...
      confirmButtonText: 'Đồng ý!'
        ...
        if (result.isConfirmed) {
          ...
          if (checkInTime.getTime() - currentTime.getTime() < TIME_CHECK) {
            this.notifi.showMessage('error', 'Canceled!', 'Không thể hủy do thời gian đến khi checkin còn nhỏ hơn 1 ngày');
          } else {
            this.orderService.customerChangeStatusOrderCanceled(id).subscribe(() => {
              this.notifi.showMessage('success', 'Canceled!', 'Hủy đặt phòng thành công');
              this.get5OrderByOrderIdRent();
            });
          }
        }
